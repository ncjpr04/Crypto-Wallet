...
function Socials() {
    const [expandedSocials, setExpandedSocials] = useState({});
    const toggleSocials = (index) => {
        setExpandedSocials((prevState) => ({
            ...prevState,
            [index]: !prevState[index],
        }));
    };
    return (
        <div className="fixed z-50 max-w-10 flex my-[14vh] pointer-events-auto">
            <div className="flex gap-3 items-center w-16 justify-center flex-col mx-3 ">
                <Button
                    onClick={() => toggleSocials(0)}
                    variant="ghost"
                    className={`text-lg  bg-black text-white`}
                >
                    {true ? 
                        <ChevronUp className="h-5 w-5 bg-gr" />
                    : 
                        <ChevronDown className="h-5 w-5" />
                    }
                </Button>
                {expandedSocials[0] && (
                    <div className=" bg-zinc-700 bg-opacity-30 backdrop-blur-sm rounded-full py-5 px-[0.5vw] font-neueMontreal flex flex-col gap-4 justify-between items-center shadow-lg ring-1 ring-black/5">
                        <button class="Btn">
